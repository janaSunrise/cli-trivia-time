...
from .api import _get_json
from .utils import (
    evaluate_score,
    generate_quiz_questions,
    get_clean_question_type,
    get_bright_color,
    get_color,
)
...
trivia_response = _get_json(
    {"amount": question_count, "difficulty": difficulty, "type": question_type}
)["results"]
...
color = (
    get_bright_color("RED")
    if score < (len(trivia_response) / 2)
    else get_bright_color("GREEN")
)
print(
    f"You scored {color}{score} {get_color('RESET')}out of {get_bright_color('LIGHTBLUE_EX')}{len(trivia_response)}."
)
